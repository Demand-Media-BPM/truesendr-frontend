...
export default function FileCleaner() {
  ...
  const handleDrop = (e) => {
    ...
    if (name.endsWith(".xlsx") || name.endsWith(".xls") || name.endsWith(".csv")) {
      ...
      toastError("Please drop a valid .xlsx / .xls / .csv file");
    }
  };
  ...
  const goNext = () => {
    if (!file) {
      toastWarning("Please upload a file first.");
      return;
    }
    setStep("rules");
  };
  ...
  async function handleDownload(type) {
    if (!jobId) {
      toastWarning("No cleaned job found yet. Please clean a file first.");
      return;
    }
