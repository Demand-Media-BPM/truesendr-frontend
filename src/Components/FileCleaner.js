...
//   // ✅ download loader state: "clean" | "invalid" | "duplicates" | null
...
//     // ✅ prevent double click / parallel downloads
...
//                     disabled={!jobId || !!downloadingType}
//                     aria-busy={downloadingType === "clean"}
//                   >
//                     <span className="fcui-btnText">
//                       {downloadingType === "clean"
//                         ? "Downloading..."
//                         : "Download Cleaned File"}
//                     </span>
//                     <span className="fcui-btnIcon" aria-hidden>
//                       {downloadingType === "clean" ? (
//                         <SpinnerIcon />
//                       ) : (
//                         <DownloadIcon />
...
//                     disabled={!jobId || !!downloadingType}
//                     aria-busy={downloadingType === "invalid"}
//                   >
//                     <span className="fcui-btnText">
//                       {downloadingType === "invalid"
//                         ? "Downloading..."
...
//                     disabled={!jobId || !!downloadingType}
//                     aria-busy={downloadingType === "duplicates"}
//                   >
//                     <span className="fcui-btnText">
//                       {downloadingType === "duplicates"
//                         ? "Downloading..."
//                         : "Download Duplicates"}
//                     </span>
//                     <span className="fcui-btnIcon" aria-hidden>
...
export default function FileCleaner() {
  ...
  // ✅ which download is currently running: "clean" | "invalid" | "duplicates" | null
  const [downloadingType, setDownloadingType] = useState(null);
  ...
  async function handleCleanClick(e) {
    ...
    try {
      ...
      const res = await axios.post(`${apiBase()}/file-cleaner/clean`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
          ...
          <aside className="fcui-left">
            ...
            {step === "picked" && (
              <>
                ...
                <div className="fcui-pickedWrap">
                  <div className="fcui-pickedPill" title={file?.name || ""}>
                    <div className="fcui-pickedName">{file?.name || "File"}</div>
                    <div className="fcui-pickedMeta">
                      {file ? prettySize(file.size) : ""}
                    </div>
            ...
            {step === "rules" && (
              <>
                <div className="fcui-rulesTop">
                  <button type="button" className="fcui-backBtn" onClick={goBack}>
                    <ArrowBackRoundedIcon fontSize="small" />
                  </button>
                ...
                <div className="fcui-rulesWrap">
                  ...
                  <div className="fcui-ruleGroup">
                    ...
                    <CheckRow
                      checked={options.removeHighRiskDomains}
                      onChange={() => handleOptionToggle("removeHighRiskDomains")}
                      title="Remove high risk domains"
                      sub="Based on your env high risk / blacklist domains"
                    />
                  </div>
            ...
            {!hasStats ? (
              ...
              <div className="fcui-resultCard">
                <div className="fcui-resultTop">
                  <div className="fcui-resultMeta">
                    <div className="fcui-resultFile">{fileLabel}</div>
                    <div className="fcui-resultDate">Cleaned on: {fmtDT(cleanedAt)}</div>
                  </div>
                </div>
                ...
                <div className="fcui-statGrid">
                  <StatBox label="Total Rows" value={stats?.totalRows ?? 0} />
                  <StatBox label="Clean Rows" value={stats?.cleanRows ?? 0} />
                  <StatBox label="Duplicates" value={stats?.removedDuplicates ?? 0} />
                  <StatBox label="Invalid" value={stats?.removedInvalidFormat ?? 0} />
                  <StatBox label="Role-based" value={stats?.removedRoleBased ?? 0} />
                  <StatBox label="Free-domain" value={stats?.removedFreeDomains ?? 0} />
                  <StatBox label="Fake" value={stats?.removedFakeLooking ?? 0} />
                  <StatBox label="High-risk" value={stats?.removedHighRiskDomains ?? 0} />
                </div>

                {/* ✅ FINAL download row (no nested div, clean layout) */}
                <div className="fcui-downloadRow">
                  <button
                    type="button"
                    className="fcui-downloadPrimary"
                    ...
                    aria-busy={downloadingType === "clean"}
                  >
                    <span className="fcui-btnText">Download Cleaned File</span>
                    <span className="fcui-btnIcon" aria-hidden>
                      {downloadingType === "clean" ? <SpinnerIcon /> : <DownloadIcon />}
                    </span>
                  </button>
                  ...
                  <button
                    ...
                    aria-busy={downloadingType === "invalid"}
                  >
                    <span className="fcui-btnText">Download Invalid Rows</span>
                    <span className="fcui-btnIcon" aria-hidden>
                      {downloadingType === "invalid" ? <SpinnerIcon /> : <DownloadIcon />}
                    </span>
                  </button>
                  ...
                  <button
                    ...
                    aria-busy={downloadingType === "duplicates"}
                  >
                    <span className="fcui-btnText">Download Duplicates</span>
                    <span className="fcui-btnIcon" aria-hidden>
                      {downloadingType === "duplicates" ? (
                        <SpinnerIcon />
                      ) : (
